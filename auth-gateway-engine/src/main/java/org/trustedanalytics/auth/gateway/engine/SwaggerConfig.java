...
public class SwaggerConfig {
  ...
  Docket orgsApi() {
    return new Docket(DocumentationType.SWAGGER_2).ignoredParameterTypes(Authentication.class)
        .apiInfo(apiInfo()).select().paths(orgsPaths()).build().useDefaultResponseMessages(false);
  }

  private Predicate<String> orgsPaths() {
    return or(regex("/organizations/.*"), regex("/users/.*"), regex("/synchronize.*"),
        regex("/state.*"), regex("/jobs.*"));
  }

  private ApiInfo apiInfo() {
    return new ApiInfoBuilder().title("Auth-gateway API")
        .description(
